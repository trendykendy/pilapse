...
EOF"
  ...
  # rewrite cron line with time restrictions
...
EOF"
  ...
  echo "Cron job now runs every ${NEW_INTERVAL} minutes from ${START_HOUR:-7}:00 to ${STOP_HOUR:-19}:00"
  exit 0
}
...
EOF"
  ...
  # install/update cron job for photo capture with time restrictions
  sudo bash -c "cat > '$CRON_FILE' <<EOF
# timelapse job: runs every ${INTERVAL_MINS} minutes between ${START_HOUR}:00 and ${STOP_HOUR}:00
*/${INTERVAL_MINS} ${START_HOUR}-${CRON_STOP_HOUR} * * * root /usr/local/bin/timelapse capture >/var/log/timelapse.log 2>&1
EOF"
  sudo chmod 644 "$CRON_FILE"
  echo "Cron job installed: runs every ${INTERVAL_MINS} minutes from ${START_HOUR}:00 to ${STOP_HOUR}:00 (see $CRON_FILE)"

  # Install end-of-day cron jobs
  echo
  echo "Installing end-of-day cron jobs..."
  
  (crontab -l 2>/dev/null | grep -v "timelapse end_of_day_sync" | grep -v "timelapse create_daily_montage" | grep -v "timelapse cleanup_directories" || true; cat << EOF

# Timelapse end-of-day tasks
$SYNC_MIN $SYNC_HOUR * * * /usr/local/bin/timelapse end_of_day_sync
$MONTAGE_MIN $MONTAGE_HOUR * * * /usr/local/bin/timelapse create_daily_montage
$CLEANUP_MIN $CLEANUP_HOUR * * * /usr/local/bin/timelapse cleanup_directories
EOF
  ) | sudo crontab -
  ...
  echo "  - Photo capture: /etc/cron.d/timelapse"
  echo "  - End-of-day tasks: root crontab"
  echo
...
Commands:
  setup                    - Initial setup (project, USB, interval)
